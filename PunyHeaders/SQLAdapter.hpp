...
public:
   ...
   vector<DataType> readColumn(const string & table_name, const string & column_name) {
      ...
      while (column_name != toString(sqlite3_column_name(statement, column_number))) {
         column_number++;
         if (column_number >= getColumnCount())
            throw runtime_error("Column " + column_name + " not found where expected.");
      }

      // Cycle through all the rows available.
      while (sqlite3_step(statement) == SQLITE_ROW) {
         const unsigned char * data  = (sqlite3_column_text(statement, column_number));

         try {
            if (data != 0)
               column_data.push_back(boost::lexical_cast<DataType>(data));
            else // Replace null value with zero.
               column_data.push_back(boost::lexical_cast<DataType>('0'));
         } catch(boost::bad_lexical_cast & e) {
            cerr << "Converting \"" << data << "\" into " << demangle(typeid(DataType)) << " has failed with " << e.what();
         }
      }
