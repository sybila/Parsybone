...
public:
	...
	void startClock(const string clock_name) {
		...
		output_streamer.output(verbose_str, "Started clock ", OutputStreamer::no_newl).output(clock_name, OutputStreamer::no_newl).output(".");
	}
	...
	void ouputClock(const string clock_name) const {
		...
		if (clocks.find(clock_name) != clocks.end()) {
			...
			output_streamer.output(verbose_str, "Requested clock ", OutputStreamer::no_newl).output(clock_name, OutputStreamer::no_newl).output(" have not been started until now.");
		}
	}
